...
final class ContrastFixer implements PaletteFixerInterface
{
     ...
     * false, it sweeps hue offsets, nearest first, at whichever chroma the
     * previous step used. The first passing candidate wins; otherwise the
     * best-effort result closest to what was actually tried is returned.
     *
    ...
    private function adjustColorContrast(
        ...
        if (!$preserveChroma) {
            ...
            $best = $relaxed;
            $chromaForHueSearch = 0.0;
        }

        if (!$preserveHue) {
            foreach (self::hueSearchOffsets() as $offset) {
                [$candidate, $candidatePassed] = $this->searchLightness($oklch, $chromaForHueSearch, $oklch->h + $offset, $background, $minContrast, $shouldLighten, $maxIterations);
                if ($candidatePassed) {
                    return $candidate;
                }
            }
